...
# 1. Verify the loop's scoring leg — no LM calls, no API key, CI-safe:
python3 optimizer/optimize.py --dry-run

# 2. Full optimization (needs an LM):
pip install -r optimizer/requirements.txt
export ANTHROPIC_API_KEY=…            # or any litellm-supported provider via --model
python3 optimizer/optimize.py --auto light            # propose + score, print result
python3 optimizer/optimize.py --auto light --apply    # …and splice the winner into the skill
```
