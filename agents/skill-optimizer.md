...
4. **Optimize.** `python3 optimizer/optimize.py --auto light --page <snap1> --page <snap2> --apply` for the fixer; `python3 optimizer/optimize.py --target adjudicator --auto light --apply` for the evaluator's judgment instructions. Start light; only escalate budget if the user asks. Warn the user about cost before running (each metric call = one LM generation + one browser evaluation).
